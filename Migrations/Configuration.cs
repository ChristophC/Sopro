    ...
    internal sealed class Configuration : DbMigrationsConfiguration<WebApplication1.Models.MyDbContext>
    {
        ...
        protected override void Seed(WebApplication1.Models.MyDbContext context)
        {
            ...
            context.ContractKinds.AddOrUpdate(
                   p => p.Description,
                   new ContractKind { Description = "Kaufverträge" },
                   new ContractKind { Description = "Mietverträge" },
                   new ContractKind { Description = "Leasingverträge" },
                   new ContractKind { Description = "Dienstleistungsvertrag" },
                   new ContractKind { Description = "Kooperationsvereinbarungen" },
                   new ContractKind { Description = "Konsiliarverträge" }
               );

            context.ContractStatuses.AddOrUpdate(
                   p => p.Description,
                   new ContractStatus { Description = "incomplete" },
                   new ContractStatus { Description = "activ" },
                   new ContractStatus { Description = "repealed" },
                   new ContractStatus { Description = "closed" },
                   new ContractStatus { Description = "deleted" }
               );

            context.ContractTypes.AddOrUpdate(
                  p => p.Description,
                  new ContractType { Description = "IT-Verträge" },
                  new ContractType { Description = "Telekommunikation" },
                  new ContractType { Description = "Liegenschaftsverwaltung" },
                  new ContractType { Description = "Fahrzeuge" },
                  new ContractType { Description = "Versicherungen" }
              );
